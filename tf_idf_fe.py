#
# Guide: Script for Feature Engineering for an SVM Model (Modular Version)
#
# This script is a template designed to guide you through the process of taking your
# clean, segmented corpus and converting it into a numerical feature matrix
# that can be used to train a classic machine learning model like an SVM.
#
# The main goal is to combine different types of features (lexical, structural,
# and syntactic) into a single, unified feature vector for each text segment.
#
...
def extract_ngram_features(processed_data):
    """
    Generates TF-IDF n-gram features from the entire corpus of text segments.
    - PARAMETER: corpus (pandas Series) - A pandas Series containing all text segments.
    - RETURNS: A sparse matrix where each row is a segment and each column is an n-gram feature.
    """
    # TODO:
    # 1. Initialize TfidfVectorizer. A good starting point is `ngram_range=(1, 2)`
    #    and `max_features=3000` to keep the size manageable.
    # 2. Use the .fit_transform() method on your corpus to create the feature matrix.
    # 3. Return the resulting sparse matrix.
    vectorizer = TfidfVectorizer(ngram_range=(1, 2), max_features=3000, lowercase=True)
    ngram_features = vectorizer.fit_transform(processed_data)
    joblib.dump(vectorizer, "tf_idf_vectorizer_combined.joblib") 
...
def extract_lexicon_features(processed_data:pd.DataFrame):
    ...
    # 3. For each text, count how many words are in your ARGUMENTATIVE_INDICATORS set.
    # 4. Append the count to your list.
...
def extract_structural_features(processed_data:pd.DataFrame):
    ...
    - PARAMETER: df (pandas DataFrame) - The full DataFrame, which should include columns
               for essay_id, sentence_index, and total_sentences_in_essay.
    - RETURNS: A numpy array of structural features for each segment.
    - NOTE: This requires you to have calculated this info during data processing!
    """
    # TODO:
    ...
    #    c. Normalized position in essay (this would require you to add essay structure
    #       information to your CSV in the data processing stage).
...
def extract_syntactic_features(processed_data:pd.DataFrame):
    ...
    # 5. Append a dictionary of these counts to your list.
...
def main():
    ...
    # TODO: Use pd.read_csv() to load your `train_corpus.csv`.
    # For this guide, we'll create a dummy DataFrame.
    processed_data = pd.read_csv(COMBINED_TRAINING_CSV_PATH)
    processed_data_df = pd.DataFrame(processed_data)
    print(f"Loaded {len(processed_data_df)} segments from {COMBINED_TRAINING_CSV_PATH}")
    ...
    # TODO: Call your other feature extraction functions here when you've implemented them.
    ...
    # TODO: When you have all your feature matrices, you will combine them here.
    ...
    # --- Final Output ---
    # You now have `final_feature_matrix` (your X) and `labels` (your y).
    # These are the inputs you will feed into your SVM model for training.
    save_npz("tf_idf_final_features_train_combined.npz", final_feature_matrix)
    np.save("tf_idf_final_labels_train_combined.npy", labels)
